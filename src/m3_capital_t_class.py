    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        h_left_x = intersection_center.x - (width / 2)
        h_left_y = intersection_center.y + (letter_thickness / 2)
        h_right_x = intersection_center.x + (width / 2)
        h_right_y = intersection_center.y - (letter_thickness / 2)

        h_left = rg.Point(h_left_x, h_left_y)
        h_right = rg.Point(h_right_x, h_right_y)
        self.h_rect = rg.Rectangle(h_left, h_right)

        v_left_x = intersection_center.x - (letter_thickness / 2)
        v_left_y = intersection_center.y - (letter_thickness / 2)
        v_right_x = intersection_center.x + (letter_thickness / 2)
        v_right_y = intersection_center.y + (height - (letter_thickness / 2))

        v_left = rg.Point(v_left_x, v_left_y)
        v_right = rg.Point(v_right_x, v_right_y)
        self.v_rect = rg.Rectangle(v_left, v_right)
    ...
    def clone(self):
        ...
        intersection_point_x = left_h.x + (width / 2)
